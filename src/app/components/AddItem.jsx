        ...
        <div className='w-full px-2 md:px-5 space-y-5 bg-white rounded-lg shadow-md p-3 border border-black'>
            ...
            <div className='w-full flex flex-col gap-2 md:grid md:grid-cols-3 '>
                {input?.map((inp, index)=>{
                    ...
                    inputElement =  <input type={inp.type} name={inp.key} value={inputData[inp.key]} className='border border-gray-400 w-full rounded-sm outline-none duration-150 px-1 focus:border-blue-400' onChange={(e)=>handleChangeInputData(e)} />
                    } else
                    if(inp.type === "select"){
                        if(inp.key === 'idKategori'){
                            inputElement =  
                            <select name={inp.key} onChange={(e)=>handleChangeInputData(e)} value={inputData[inp.key]} className='border border-gray-400 w-full rounded-sm outline-none duration-150 px-1 focus:border-blue-400'>
