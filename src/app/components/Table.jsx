...
const Table = ({ field, row }) => {
  ...
  return (
    <>
    <div className='border-2 border-black rounded-2xl overflow-hidden bg-white my-2'>
        <section id='desktop' className='hidden md:block'>
            <table className='w-full text-center'>
                ...
                <tbody>
                    {
                        row?.map((data, index)=>(
                            <tr key={index} className={`${index % 2 === 0 ? 'bg-blue-200' : 'bg-green-200'} h-7`}>
                                <td>{index+1}</td>
                                    {
    ...
    <div className={`w-full h-full bg-black fixed top-0 left-0 bg-opacity-60 px-2 flex items-center justify-center ${isShowDetail ? 'visible' : 'invisible'}`} onClick={handleClickShowDetail}>
        <div className={`w-full bg-white duration-200 p-5 space-y-5 rounded-lg ${isShowDetail ? 'scale-100' : 'scale-0'}`}>
            <h3 className='text-lg font-semibold text-center'>Detail Barang</h3>
                {
                    detailBarang.map(data=>(
                        <div className=''>
                            {
                                field.map((value, index)=>(
                                    <div key={index} className='flex gap-5'>
                                        <div className='w-28 font-semibold px-2'><p>{value.label}</p></div>
                                        <div className='flex-1'><p>: {data[value.key]}</p></div>
                                    </div>
                                )).slice(1)
                            }
                        </div>
