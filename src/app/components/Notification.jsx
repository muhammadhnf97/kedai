...
const Notification = ({ desc, alertTitle, handleClickResponseNotif }) => {
    ...
    if(alertTitle === 'caution'){
        answer = ( <button autoFocus className='w-20 px-2 py-1 bg-green-400 rounded-md shadow-md h-full hover:bg-green-500 active:bg-green-600' onClick={()=>handleClickResponseNotif(false)}>Ok</button> )
    } else {
        answer = (
    ...
    <div className='fixed w-full h-full bg-black flex items-center justify-center top-0 bg-opacity-60 z-30 p-5'>
        <div className='w-80 h-40 bg-white rounded-lg overflow-hidden flex flex-col'>
            <div className='w-full flex items-center justify-center bg-blue-200 py-3 gap-2'>
                { alertTitle === 'warning' ? <ImWarning className='text-red-600 w-6 h-6'/> : <AiOutlineInfoCircle className='w-6 h-6 text-blue-600' /> }
